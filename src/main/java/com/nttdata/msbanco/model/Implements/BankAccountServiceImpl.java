...
public class BankAccountServiceImpl implements BankAccountService {
  ...
  public Mono<BankAccount> saveBankAccount(BankAccount bankAccount) {
    ...
    Mono<BankAccount> bankAccountSaved =
        validateSave(bankAccount)
            .flatMap(
                result -> {
  ...
  private Mono<Boolean> validateSave(BankAccount bankAccount) {
    ...
    Mono<Boolean> isValidated =
        customerMono.flatMap(
            x -> {
              log.info(
                  "Datos de Cliente: " + x.getName() + " " + x.getLastName() + " | " + x.getDni());

              Flux<BankAccount> bankAccountList = getAllBankAccountsByCustomer(x.getDni());
              List<BankAccount> bankAccounts =
                  bankAccountList.toStream().collect(Collectors.toList());
